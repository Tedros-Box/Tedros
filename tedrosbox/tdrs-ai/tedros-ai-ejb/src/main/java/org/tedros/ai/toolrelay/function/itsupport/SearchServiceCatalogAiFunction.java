...
/**
 * Versão de backend da {@code SearchServiceCatalogAiFunction} do FE
 * (app-itsupport-tools-fx) — name, description, strings de resultado e a
 * lógica de filtragem copiados literalmente; a chamada ao controller usa
 * lookup JNDI server-side com o token do request corrente.
 *
 * @author Davis Gordon
 */
@ApplicationScoped
public class SearchServiceCatalogAiFunction implements TServerAiFunction {
    ...
    public Class<?> getModel() {
        return ServiceCatalogFilterModel.class;
    }

    @Override
    public Object execute(Object arg, TAiToolContext ctx) {
        ServiceCatalogFilterModel filter = (ServiceCatalogFilterModel) arg;

        try {
            TResult<List<ServiceCatalog>> result = search(ctx.getToken());
    ...
    protected TResult<List<ServiceCatalog>> search(TAccessToken token) throws Exception {
        TServiceLocator serv = TServiceLocator.getInstance();
        try {
            IServiceCatalogController controller = serv.lookupWithRetry(IServiceCatalogController.JNDI_NAME);
            return controller.search(token, (String) null);
        } finally {
            serv.close();
        }
    }
